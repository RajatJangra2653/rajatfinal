...
$password = ConvertTo-SecureString "Rjangra#2021" -AsPlainText -Force
...
# Register the scheduled task with authentication
Register-ScheduledTask -TaskName ABC -Action $action -Trigger $trigger -User $username -Password $password -TaskPath "\" -Credential $credential
